...
Authentication routes are not activated in the API process until the focused [HTTPS attestation decision](../planning/auth-be-003-https-attestation-blocker.md) defines how the private API authenticates the original browser scheme through that proxy. `X-Forwarded-Proto` is not trusted merely because it is present.
