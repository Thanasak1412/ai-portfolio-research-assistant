...
The final implementation uses [AUTH_IMPLEMENTATION_POLICY-v2](../policies/AUTH_IMPLEMENTATION_POLICY-v2.md). It mitigates credential stuffing and brute force with shared limits; account enumeration with generic failures; replay with atomic row-lock rotation and family revocation; token forgery with a fixed EdDSA-only verifier and validated claims; XSS exposure by memory-only access tokens; CSRF with same-origin HTTPS, Origin, and custom-header checks; proxy spoofing with explicit trusted CIDRs; and logging leakage with allowlisted fields and redaction tests. `AUTH_IMPLEMENTATION_POLICY-v1` remains the historical pre-closure composition.
