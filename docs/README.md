# Documentation Index

## Approved planning and decisions

- [Planning Baseline](planning/planning-baseline.md)
- [Portfolio Foundation Decision Closure Specification](planning/decision-closure-specification.md)
- [Authentication Phase 1 — Execution Plan](planning/authentication-phase-1-execution-plan.md)
- [Authentication Security Decision Closure Specification](planning/authentication-security-decision-closure-specification.md)
- [Authentication Implementation Gate](planning/authentication-implementation-gate.md)
- [Portfolio & Asset Foundation — Execution Plan](planning/portfolio-asset-foundation-execution-plan.md) (approved; M2-PLAN-001)
- [AUTH-BE-002 Security Decision Gaps](planning/auth-be-002-implementation-blockers.md)
- [AUTH-BE-003 HTTPS Attestation Decision](planning/auth-be-003-https-attestation-blocker.md) (resolved; runtime activation implemented in AUTH-BE-003A, pending review)
- [Argon2id Benchmark Report — 2026-08-04](planning/argon2id-benchmark-report-2026-08-04.md)
- [AUTH_IMPLEMENTATION_POLICY-v3](policies/AUTH_IMPLEMENTATION_POLICY-v3.md) (current)
- [AUTH_IMPLEMENTATION_POLICY-v2](policies/AUTH_IMPLEMENTATION_POLICY-v2.md) (historical)
- [AUTH_IMPLEMENTATION_POLICY-v1](policies/AUTH_IMPLEMENTATION_POLICY-v1.md) (historical)
- [PASSWORD_HASH-v1](policies/PASSWORD_HASH-v1.md)
- [TOKEN_SIGNING-v1](policies/TOKEN_SIGNING-v1.md)
- [AUTH_BROWSER_SECURITY-v1](policies/AUTH_BROWSER_SECURITY-v1.md)
- [CLIENT_NETWORK_IDENTITY-v1](policies/CLIENT_NETWORK_IDENTITY-v1.md)
- [AUTH_RATE_LIMIT-v1](policies/AUTH_RATE_LIMIT-v1.md)
- [REFRESH_TOKEN-v1](policies/REFRESH_TOKEN-v1.md)
- [AUTH_HMAC_KEYS-v1](policies/AUTH_HMAC_KEYS-v1.md)
- [HTTPS_ATTESTATION-v1](policies/HTTPS_ATTESTATION-v1.md)

## ADRs

- [ADR-000 — Bootstrap Repository Boundary](adr/ADR-000-bootstrap-repository.md)
- [Authentication Frontend Runtime](engineering/authentication-frontend-runtime.md)
- [Authentication Local HTTPS](engineering/authentication-local-https.md)
- [Authentication Operations Runbook](engineering/authentication-operations-runbook.md)
- [Authentication M1 Completion Report](engineering/authentication-m1-completion-report.md) (AUTH-VERIFY-001; closed)
- [Authentication M1 Acceptance Matrix](engineering/authentication-m1-acceptance-matrix.md)
- [M2 Real-stack Browser Verification](engineering/m2-real-stack-e2e.md)
- [M2 Portfolio & Asset Acceptance Matrix](engineering/m2-portfolio-asset-acceptance-matrix.md) (M2-VERIFY-001; pending final verification PR)
- [M2 Portfolio & Asset Completion Report](engineering/m2-portfolio-asset-completion-report.md) (M2-VERIFY-001; pending final verification PR)
- [ADR-013 — Solo Maintainer Merge Governance](adr/ADR-013-solo-maintainer-merge-governance.md)
- [ADR-014 — Argon2id Password Hashing Parameters](adr/ADR-014-argon2id-password-hashing.md)
- [ADR-015 — Ed25519 Key Ring and Secret Management](adr/ADR-015-ed25519-key-ring-and-secret-management.md)
- [ADR-016 — Authentication Browser and Cookie Topology](adr/ADR-016-authentication-browser-and-cookie-topology.md)
- [ADR-017 — Trusted Proxy and Client IP Derivation](adr/ADR-017-trusted-proxy-and-client-ip-derivation.md)
- [ADR-018 — Distributed Authentication Rate Limiting](adr/ADR-018-distributed-authentication-rate-limiting.md)
- [ADR-019 — Refresh Token Representation and Digest](adr/ADR-019-refresh-token-representation-and-digest.md)
- [ADR-020 — Authentication HMAC Key Representation and Derivation](adr/ADR-020-authentication-hmac-key-representation-and-derivation.md)
- [ADR-021 — Trusted HTTPS Scheme Attestation](adr/ADR-021-trusted-https-scheme-attestation.md)

## Operational contracts

- [Authentication Database Ownership](architecture/authentication-database.md)
- [Portfolio and Asset Database Ownership](architecture/portfolio-asset-database.md)
- [Authentication Local HTTPS](engineering/authentication-local-https.md)
- [Authentication Deployment Contract](engineering/authentication-deployment-contract.md)
- [Authentication Backend Runtime](engineering/authentication-backend-runtime.md)
- [M0 Closure Report](m0-blocker-resolution-and-closure-report-2026-08-03.md)
